function [sinogram, angles, data] = skyscan_rebin_fan2par(Datafilename, logfilename, dets)
%------------------------------------------------------------------------
...
sinogramNew = flip(sinogramBg',1);%/SinoMax;
 
%if isfield(data,'postalignment')
%	sinogramNew = circshift(sinogramNew, [0 data.postalignment]);
%end
...
if nargin < 3
	...
	F = rebin_fan2par(sinogramNew, BetaDeg, D, ThetaDeg, dets);
end
F(F < 0) = 0;
